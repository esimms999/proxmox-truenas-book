...
The same host can pass a second device — a discrete GPU — into another VM using the identical BIOS/IOMMU/vfio pipeline; see @sec-gpu. The GPU and the HBA just need to land in separate IOMMU groups.
...
- If also passing a GPU: a board with an iGPU so the host keeps a console, or a plan to run headless (@sec-gpu)
